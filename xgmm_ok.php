...
if($_SESSION['online_number']==true){
	...
	$query=mysql_query("select * from tb_user where online_number='$number' and online_pass='$pass'");
	if(mysql_num_rows($query)<1){
		echo "<script>alert('您输入的准考证号码和密码不符，请重新输入！'); window.location.href='index.php?online=修改密码';</script>";
	}else{
		$querys=mysql_query("update tb_user set online_pass='$passes' where online_number='$number'");
		if($querys){
